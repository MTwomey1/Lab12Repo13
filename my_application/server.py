...
@app.route("/queues/<name>", methods=['POST'])
def queues_create():
	"""
	Create queue
	curl -X POST -H 'Content-Type: application/json' http://localhost:8080/queues -d '{"name": "<name>"}'
...
def queues_remove(name):
	"""
	Delete queue
	curl -X DELETE -H 'Accept: application/json' http://localhost:8080/queues/<queue>
...
def messages_read(name):
	...
	curl -X GET -H 'Accept: application/json' http://localhost:8080/queues/<queue>/msgs
	"""
...
def get_conn():
	key_id, secret_access_key = urllib2.urlopen("ec2-35-160-32-177.us-west-2.compute.amazonaws.com:81/key").read().split(':')
	return boto.sqs.connect_to_region("us-west-2", aws_access_key_id=key_id ,aws_secret_access_key=secret_access_key)
